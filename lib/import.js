...
const onLine = line => {
  ...
  if (!isJsonLine(line)) {
    logger.error(line, `invalid line: ${lineCount}`)
    return
  }
...
var formatAndPutCurrentBatch = () => {
  let currentBatch
  logger.success(entitiesBatch.length, 'putting batch...');
  ...
  .catch(err => logger.error([ err, err.body ], 'formatAndPutBatch err body'))
}
