...
const PutNextBatch = (domain, index, type, urls) => {
  const putNextBatch = () => {
    ...
    logger.info(url, `preparing next ${type} batch`)
...
var removeMissingEntities = entities => {
  for (const id in entities) {
    const entity = entities[id]
    if (entity == null) {
      logger.warn(id, 'missing value: ignored')
      delete entities[id]
    }

    if (!(entity && entity.claims)) {
      // Known case: desambiguation pages given the type meta
      logger.warn(id, 'entity has no claims: ignored')
      delete entities[id]
    }
  }
...
var spreadIdsByDomain = (data, uri) => {
